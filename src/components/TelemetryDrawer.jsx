...
export default function TelemetryDrawer({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end overflow-hidden">
      ...
      {/* Drawer */}
      <div className="relative w-full max-w-lg bg-white dark:bg-gray-800 shadow-2xl flex flex-col h-full animate-slide-in-right transition-colors">
        {/* Header */}
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900/50">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs font-mono text-gray-500 dark:text-gray-400 uppercase tracking-tight bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">
                {vehicle.vin}
              </span>
              {timestamp && (
        ...
        {/* Search & Actions */}
        <div className="px-6 py-3 border-b border-gray-100 dark:border-gray-700 flex gap-2 overflow-hidden shrink-0 bg-white dark:bg-gray-800">
          <div className="relative flex-1">
            <input
              type="text"
              placeholder="Search by key or value..."
              className="w-full pl-9 pr-4 py-2 bg-gray-100 dark:bg-gray-700 border-transparent focus:bg-white dark:focus:bg-gray-600 focus:ring-2 focus:ring-blue-500 focus:border-transparent rounded-lg text-sm transition-all outline-none dark:text-white dark:placeholder-gray-400"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <svg
              className="w-4 h-4 text-gray-400 dark:text-gray-500 absolute left-3 top-2.5"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 disabled:bg-gray-100 dark:disabled:bg-gray-800 text-indigo-600 dark:text-indigo-400 disabled:text-gray-400 dark:disabled:text-gray-600 text-sm font-bold rounded-lg border border-indigo-100 dark:border-indigo-800 transition-all active:scale-95"
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-0 custom-scrollbar bg-gray-50/30 dark:bg-gray-900/30">
          {vehicle.isScanning ? (
            <div className="flex flex-col items-center justify-center h-64 gap-4">
              <div className="w-12 h-12 border-4 border-blue-600/20 dark:border-blue-400/20 border-t-blue-600 dark:border-t-blue-400 rounded-full animate-spin"></div>
            ...
            <div className="bg-white dark:bg-gray-800">
              {groupedData.map((group, gIdx) => (
                <div key={gIdx} className="mb-4">
                  <div className="px-6 py-2 bg-gray-100/80 dark:bg-gray-700/80 border-y border-gray-200 dark:border-gray-600 sticky top-0 z-10">
                    <h3 className="text-xs font-black text-gray-500 dark:text-gray-300 uppercase tracking-widest leading-none">
                      {group.label}
                    </h3>
                  </div>
                  <div className="divide-y divide-gray-100 dark:divide-gray-700">
                    {group.items.map((item, idx) => (
                      <div
                        key={idx}
                        className="px-6 py-4 hover:bg-blue-50/30 dark:hover:bg-blue-900/20 transition-colors group/row overflow-hidden"
                      >
                        <div className="flex justify-between items-start gap-4">
                          <div className="min-w-0 flex-1">
                            <div className="flex-1 group/name flex items-center min-w-0">
                              ...
                              {item.units && (
                                <span className="shrink-0 text-[10px] font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded ml-2">
                                  {item.units}
                                </span>
                              )}
                            </div>
                            <div className="mt-0.5 group/alias min-w-0">
                              <p className="text-[10px] font-mono font-medium text-gray-400 dark:text-gray-500 uppercase leading-none group-hover/row:text-blue-500 dark:group-hover/row:text-blue-400 transition-colors truncate">
                                {item.alias || item.deviceKey}
                              </p>
                            </div>
                            <div className="mt-2 text-sm font-semibold text-gray-800 dark:text-gray-200 bg-gray-100/50 dark:bg-gray-700/50 p-2 rounded border border-gray-100 dark:border-gray-600 group/value truncate">
                              {String(item.value)}
                            </div>
                          </div>
                        </div>
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-[10px] text-gray-400 dark:text-gray-500 flex justify-between items-center shrink-0">
