...
export const ALL = async (context) => {
  ...
  // Retrieve secret from Cloudflare runtime env or Astro build env
  const env = locals?.runtime?.env || import.meta.env;
  const XHASH_SECRET_KEY = env.VINFAST_XHASH_SECRET;

  if (!XHASH_SECRET_KEY) {
    console.error("CRITICAL: VINFAST_XHASH_SECRET is missing!");
    return new Response(
